...
using MachO
using COFF
using NativeDebugger: find_module, SyntheticModule, ConcreteModule, Module, load,
  make_fdetab, make_inverse_symtab
import NativeDebugger: symbolicate
...
function get_word{T}(s::NativeDebugger.LocalSession, ptr::RemotePtr, ::Type{T}=NativeDebugger.intptr(NativeDebugger.getarch(s)))
    NativeDebugger.Hooking.mem_validate(UInt(ptr), sizeof(Ptr{Void})) || error("Invalid load")
    UInt64(load(s, RemotePtr{T}(ptr)))
end
get_word{T}(s, ptr::RemotePtr, ::Type{T}=NativeDebugger.intptr(NativeDebugger.getarch(s))) =
  UInt64(load(s, RemotePtr{T}(ptr)))

function find_fde(mod, modrel)
    slide = 0
    eh_frame = NativeDebugger.find_ehframes(mod)[1]
    if isa(mod, Module) && isnull(mod.ehfr)
        ...
        tab = NativeDebugger.find_ehfr(mod)
...
function probably_in_entrypoint(h, ip)
    start = NativeDebugger.X86_64.BasicRegs(deref(first(filter(x->isa(deref(x),MachO.thread_command),LoadCmds(h))))).rip[]
    start -= NativeDebugger.first_actual_segment(h).vmaddr
...
function entry_cfa(mod, r)
    ...
    regs = NativeDebugger.X86_64.inverse_dwarf
...
function compute_register_states(s, base, mod, r, stacktop, ::Void)
    ...
    #out = STDOUT #IOContext(STDOUT, :reg_map => NativeDebugger.X86_64.dwarf_numbering)
...
function evaluate_cfi_expr(opcodes, s, r, rs, cfa_addr = nothing)
    # wordT= NativeDebugger.intptr(NativeDebugger.getarch(s)), but this is hot, so we can't really afford it
    wordT = UInt64
...
#=
symbolicate(session, modules, frame::NativeDebugger.CStackFrame) =
  symbolicate(session, modules, frame.ip - (frame.stacktop?0:1))
=#
symbolicate(modules, ip) = symbolicate(NativeDebugger.LocalSession(), modules, ip)
...
function symbolicate(session, base, mod, ip, syms=NativeDebugger.get_syms(mod))
    ...
    sections = Sections(NativeDebugger.dhandle(mod))
    ...
    strtab = StrTab(syms)
    if isa(mod, NativeDebugger.Module)
        isnull(mod.inverse_symtab) &&
            (mod.inverse_symtab = Nullable(make_inverse_symtab(NativeDebugger.dhandle(mod))))
...
function symbolicate_plt_entry(symbols, section_sym, offset)
    ...
    local rel
    if name == ".plt"
        rel_sec = first(NativeDebugger.filter_named_sections(sections, "rela.plt"))
        # First entry is fake
        rel = ELF.Relocations(rel_sec)[(div(offset, 0x10)-1)+1]
    else
        @assert name == ".plt.got"
        rel_sec = first(NativeDebugger.filter_named_sections(sections, "rela.dyn"))
    ...
    dynsym = ELF.Symbols(first(NativeDebugger.filter_named_sections(sections, "dynsym")))
...
function unwind_step(s, modules, r, cfi_cache = nothing; stacktop = false, ip_only = false, allow_frame_based = true)
    ...
    elseif !isnull(mod.xpdata)
        ...
        while i <= length(entry.opcodes) 
            ...
            opcode, info = NativeDebugger.opinfo(op)
            ...
            elseif opcode == UWOP_ALLOC_LARGE || opcode == UWOP_ALLOC_SMALL
                # Undo the allocation
                size, nskip = NativeDebugger.compute_alloc_op_size(entry.opcodes, i)
...
# Win64 unwinding
using NativeDebugger: PData, RUNTIME_FUNCTION, UNWIND_CODE,
