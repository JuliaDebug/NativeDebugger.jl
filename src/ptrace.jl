module Ptrace
  ...
  using ..NativeDebugger: RemotePtr, X86_64
  using ..NativeDebugger.Registers: RegisterValue
  using ObjFileBase
  ...
  function NativeDebugger.read_exe(s::Session)
      readmeta(IOBuffer(open(Base.Mmap.mmap, "/proc/$(pid(s))/exe")))
  end
  ...
  function attach(pid)
    ...
    imageh = NativeDebugger.read_exe(sess)
    modules = NativeDebugger.GlibcDyldModules.load_library_map(sess, imageh)
    sess, modules
  end
  
  function NativeDebugger.load{T}(s::Session, ptr::RemotePtr{T})
    r = Ref{T}()
  ...
  function NativeDebugger.load{T}(s::Session, ptr::RemotePtr{T}, count)
    r = Vector{T}(count)
    Base.systemerror("pread(/proc/mem)",
      ccall(:pread, Cssize_t, (Cint, Ptr{Void}, Csize_t, UInt64),
  ...
  function NativeDebugger.mapped_file(s::Session, ptr::RemotePtr)
  ...
  const NT_PRSTATUS = 1
  function NativeDebugger.getregs(s::Session)
  ...
  function NativeDebugger.continue!(s::Session)
    ptrace(PTRACE_CONT, s.pid, C_NULL, C_NULL)
    waitpid(s)
  end
  
  function NativeDebugger.single_step!(s::Session)
